...
var addFilenames = _metalsmithFilenames2["default"]();
...
function buildPattern(metalsmith, patterns, livereload, options, previousFilesMap) {
  _unyield2["default"](metalsmith.read())(function (err, files) {
    if (err) {
      options.log(_chalk2["default"].red("" + nok + " " + err));
      return;
    }
    ...
    var filesToUpdate = {};
    _multimatch2["default"](Object.keys(files), patterns).forEach(function (path) {
...
exports["default"] = function (options) {
  ...
  if (options.livereload) {
    livereload = _livereload2["default"](options.livereload, options.log);
  }
  ...
  var plugin = function metalsmithWatch(files, metalsmith, cb) {
    ...
    Object.keys(options.paths).map(function (pattern) {
      var watchPattern = pattern.replace("${source}", _path.normalize(metalsmith._source));
      patterns[watchPattern] = options.paths[pattern];
    });

    _gaze2["default"](Object.keys(patterns), _extends({}, options.gaze, {
      cwd: metalsmith._directory }), function watcherReady(err, watcher) {
      if (err) {
        throw err;
      }
      ...
      var update = function update() {
        ...
        pathsToUpdate.some(function (file) {
          ...
          var filesToInvalidate = Object.keys(patterns).reduce(function (acc, pattern) {
            return [].concat(_toConsumableArray(acc), _toConsumableArray(_multimatch2["default"](Object.keys(require.cache), "" + _path.resolve(metalsmith._directory) + "/" + pattern)));
          }, []);
        ...
        var filesToUpdate = _multimatch2["default"](pathsToUpdate, patternsToUpdate).map(function (file) {
          return _path.relative(metalsmith._source, file);
        });
        ...
        var patternsToUpdatePattern = Object.keys(patterns).filter(function (pattern) {
          ...
          return _multimatch2["default"](pathsToUpdate, pattern).length > 0;
      ...
      watcher.on("all", function (event, path) {
        var filename = _path.relative(metalsmith._directory, path);
        ...
        if (event === "added" || event === "changed" || event === "renamed") {
          pathsToUpdate.push(_path.relative(metalsmith.path(), path));
          if (updatePlanned) {
